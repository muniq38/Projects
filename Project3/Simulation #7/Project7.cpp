...
const int width=200, height=200, micx=100, micy=155, F=523;
double alpha=0.00002, c=340, t=(double)1/44100, h=0.02, Amp=1;
...
int main()
{
  FILE *fp1=fopen("Second Part.wav","rb");
  ...
  for(int iter=0; iter<n; iter++){
	...
	for(int i=1; i<=width; i++) for(int j=1; j<=height; j++){
	  ...
	  int K=2*i+j, L=2*i-j; 
	  if(j==50 && (i<95 || 105<i)) Wall[i][j]=2;
	  if(50<j && j<150 && ((228<K&&K<232) || (168<L&&L<172))) Wall[i][j]=2;
	}
	for(int i=1; i<=width; i++) for(int j=1; j<=height; j++){
