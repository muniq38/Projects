...
const int width=200, height=200, micx=92, micy=100, micx2=108, micy2=100, F=523;
double alpha=0.00002, c=340, t=(double)1/44100, h=0.02, Amp=2;
...
int main()
{
  FILE *fp1=fopen("Third Part.wav","rb");
  ...
  for(int iter=0; iter<n; iter++){
  	// making sound here
  	int movex, movey;
  	if(iter<n/3){ movex=108; movey=100; }
  	else if(iter<2*n/3){ movex=100+8*cos(3*PI*(iter-n/3)/n); movey=100+8*sin(3*PI*(iter-n/3)/n); }
	else{ movex=92; movey=100; }
	Pixel[movex][movey]=Amp*R[iter]; // Amp : Volume up
	...
	for(int i=1; i<=width; i++) for(int j=1; j<=height; j++){
	  ...
	  int Dist = (i-100)*(i-100)+(j-100)*(j-100);
	  if(Dist<60) Wall[i][j]=2;
	}
	for(int i=1; i<=width; i++) for(int j=1; j<=height; j++){
	  ...
	  if(i==1) Pixel[i][j]=Pixel[i+1][j];
  	  ...
  	  if(Wall[i][j]==2) Pixel[i][j] *= 0.93; // Transmission Rate 
  	  if(Wall[i][j]==3) Pixel[i][j] = 0; // Full Reflection
	}
	pcm[iter][1]=Pixel[micx2][micy2];
	pcm[iter][0]=Pixel[micx][micy];
  }
