...
```
    ...
    _________________________________________________________________
    dense_1 (Dense)              (None, 10, 5)             645       
    _________________________________________________________________
    activation_1 (Activation)    (None, 10, 5)             0         
    =================================================================
...
```
    ...
    <ipython-input-36-f3c1d167b841> in <module>()
    ...
    /opt/conda/lib/python3.6/site-packages/keras/engine/training.py in fit(self, x, y, batch_size, epochs, verbose, callbacks, validation_split, validation_data, shuffle, class_weight, sample_weight, initial_epoch, steps_per_epoch, validation_steps, **kwargs)
       1519             class_weight=class_weight,
       1520             check_batch_axis=False,
    -> 1521             batch_size=batch_size)
       1522         # Prepare validation data.
       1523         if validation_data:
    ...
    /opt/conda/lib/python3.6/site-packages/keras/engine/training.py in _standardize_user_data(self, x, y, sample_weight, class_weight, check_batch_axis, batch_size)
       1379                                     output_shapes,
       1380                                     check_batch_axis=False,
    -> 1381                                     exception_prefix='target')
       1382         sample_weights = _standardize_sample_weights(sample_weight,
       1383                                                      self._feed_output_names)


    /opt/conda/lib/python3.6/site-packages/keras/engine/training.py in _standardize_input_data(data, names, shapes, check_batch_axis, exception_prefix)
    ...
    ValueError: Error when checking target: expected activation_1 to have 3 dimensions, but got array with shape (132, 5)
...
```
    ...
    <ipython-input-37-cdec026f7e15> in <module>()
          1 X_test_indices = sentences_to_indices(X_test, word_to_index, max_len = maxLen)
          2 Y_test_oh = convert_to_one_hot(Y_test, C = 5)
    ----> 3 loss, acc = model.evaluate(X_test_indices, Y_test_oh)
          4 print()
          5 print("Test accuracy = ", acc)


    /opt/conda/lib/python3.6/site-packages/keras/engine/training.py in evaluate(self, x, y, batch_size, verbose, sample_weight, steps)
       1641             sample_weight=sample_weight,
       1642             check_batch_axis=False,
    -> 1643             batch_size=batch_size)
       1644         # Prepare inputs, delegate logic to `_test_loop`.
       1645         if self.uses_learning_phase and not isinstance(K.learning_phase(), int):
    ...
    ValueError: Error when checking target: expected activation_1 to have 3 dimensions, but got array with shape (56, 5)
